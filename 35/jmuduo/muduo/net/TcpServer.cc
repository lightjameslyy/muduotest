...
TcpServer::TcpServer(EventLoop* loop/*mainreactor*/,
...
void TcpServer::setThreadNum(int numThreads)//不包含mainreactor所属线程
{
  assert(0 <= numThreads);
...
void TcpServer::start()
{
  ...
  {
    started_ = true;
	threadPool_->start(threadInitCallback_);//线程初始化的回调函数
  }

  if (!acceptor_->listenning())//如果没有处于监听状态
  {
	// get_pointer返回原生指针
    loop_->runInLoop(
...
void TcpServer::newConnection(int sockfd, const InetAddress& peerAddr)
{
  ...
  ioLoop->runInLoop(boost::bind(&TcpConnection::connectEstablished, conn));//ioLoop所属对象调用它
